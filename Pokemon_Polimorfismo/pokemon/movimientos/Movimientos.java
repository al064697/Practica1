...
public class Movimientos {
    ...
    public void setPuntosPoder(int puntosPoderAtaque) { this.puntosPoder = puntosPoderAtaque; }
    ...
    public double calcularMovimiento(Pokemon pokemon, Movimientos movimiento) {
        ...
        switch (clase) {
            case ESPECIAL:
                dannio = (0.01 * bonificacion * efectividad * pokemon.getVariacion()) * (((0.2 * pokemon.getNivel() + 1) * 
                pokemon.getPuntosAtaqueEspecial() * movimiento.getPuntosDeAtaque()) / (25 * pokemon.getPuntosDefensaEspecial())) + (2);
                break;
            case FISICO:
                dannio = (0.01 * bonificacion * efectividad * pokemon.getVariacion()) * (((0.2 * pokemon.getNivel() + 1)  * 
                pokemon.getPuntosAtaque() * movimiento.getPuntosDeAtaque()) / (25 * pokemon.getPuntosDefensa())) + (2);
                break;
        }
        return dannio;
    }
